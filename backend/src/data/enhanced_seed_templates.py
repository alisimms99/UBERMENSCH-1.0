...
def create_enhanced_exercises():
    ...
    if not has_app_context():
        raise RuntimeError("create_enhanced_exercises() must be called within an active Flask app context.")
...
def create_enhanced_workout_templates():
    ...
    if not has_app_context():
        raise RuntimeError("create_enhanced_workout_templates() must be called within an active Flask app context.")

    try:
        # Re-fetch exercises to ensure we have attached instances
        exercises = {e.name: e for e in Exercise.query.all()}
        if not exercises:
            # If separate call, try creating them (no nested app context)
        ...
        for name, phase, order, reps, sets, dur in t1_items:
            ...
            if not ex and name == 'Wall Pushups':
                ex = exercises.get('Pushups')
